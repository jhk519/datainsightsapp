...
class QueryPanel(AppWidget):
    def __init__(self,parent,controller,config,dbvar=None):
        ...
        self.build_search_menu(self.search_buttons_labelframe)
    ...
    def generate_current_selections_pack(self,which_axis="left"):
        return {
            ...
            "category_or_product":self.category_or_product_var.get(),
            "category_or_product_entry":self.category_or_product_entry_var.get(),
    ...
    def reset_vars(self):
        self.category_or_product_var.set("DEFAULT_IGNORE"),
        self.category_or_product_entry_var.set("DEFAULT_IGNORE"),
    ...
    def build_skeleton(self):
        ...
        self.search_buttons_labelframe = tk.LabelFrame(self,text="Search")
        self.search_buttons_labelframe.grid(row=5,column=0,sticky="news")
    ...
    def build_start_end_data_filter(self,targ_frame,rown=0):
        ...
        ttk.Button(targ_frame,text="<<",width=3,command=lambda: self._ux_skip_calendars(-30)).grid(
                row=rown,column=0,pady=2,sticky="w")
        ttk.Button(targ_frame,text=">>",width=3,command=lambda: self._ux_skip_calendars(30)).grid(
                row=rown, column=1, pady=2)         
...
#        available_colors = self.get_cfg_val("colors_preferred").split("-")
    ...
    def build_search_menu(self,targ_frame,rown=0):
